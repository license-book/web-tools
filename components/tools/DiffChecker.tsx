...
export default function DiffChecker() {
  ...
  const diff = useMemo(() => {
    if (!run) return [] as DiffLine[];
    try { setError(''); return diffLines(left, right); } catch (e) { setError(e instanceof Error ? e.message : '비교할 수 없습니다.'); return []; }
  }, [left, right, run]);
  ...
  return <div className="workspace">
    <div className="jsonEditorGrid">
      <div className="jsonPane"><span className="fieldLabel">원본</span><textarea className="toolTextarea jsonTextarea" value={left} onChange={(e) => { setLeft(e.target.value); setRun(false); }} placeholder="원본 텍스트 또는 코드를 입력하세요." /></div>
      <div className="jsonPane"><span className="fieldLabel">변경본</span><textarea className="toolTextarea jsonTextarea" value={right} onChange={(e) => { setRight(e.target.value); setRun(false); }} placeholder="비교할 텍스트 또는 코드를 입력하세요." /></div>
    </div>
    <div className="workspaceAction"><button className="primaryButton" type="button" onClick={() => setRun(true)}>차이 비교</button><span>추가 {counts.add}줄 · 삭제 {counts.remove}줄</span><button className="textButton" type="button" onClick={() => { setLeft(''); setRight(''); setRun(false); }}>초기화</button></div>
    {error && <div className="engineError">{error}</div>}
    {run && !error && <div className="resultPanel"><div className="workspaceHead"><strong>라인 비교 결과</strong><span>+ 추가 · − 삭제</span></div><div style={{ maxHeight: 520, overflow: 'auto', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: 13 }}>{diff.map((line, idx) => <div key={idx} style={{ padding: '7px 14px', whiteSpace: 'pre-wrap', wordBreak: 'break-word', background: line.type === 'add' ? '#ecfdf5' : line.type === 'remove' ? '#fff1f2' : '#fff', color: line.type === 'add' ? '#047857' : line.type === 'remove' ? '#b91c1c' : '#475569', borderBottom: '1px solid #f1f5f9' }}>{line.type === 'add' ? '+' : line.type === 'remove' ? '−' : ' '} {line.text || ' '}</div>)}</div></div>}
  </div>;
}
